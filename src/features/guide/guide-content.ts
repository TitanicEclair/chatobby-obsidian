...
Compaction writes a smaller continuity checkpoint before work continues. If the threshold is reached during a long tool-driven turn, Chatobby waits for the current tool result, checkpoints at that safe boundary, and resumes the same request from a continuation handoff. It does not terminate or replay the operation that reached the boundary. A message sent during automatic compaction is accepted once and waits behind that checkpoint; it should not show a false 30-second prompt timeout or start a duplicate turn. When compaction finishes, the feed should show **Context compacted**, then the resumed work or queued message and response in order.

The checkpoint has defined outcomes: full fidelity keeps useful active working context, continuity safe drops optional items while preserving unresolved requests and work, and host-reconciled minimum conservatively rebuilds continuity if bounded model corrections are exhausted. The context meter may briefly say that it is calculating while the checkpoint is installed, but it should refresh to the post-compaction usage without showing the old pre-compaction total. Cancellation or provider failure leaves the original context usable. If the meter remains empty, the queued message appears twice, or the feed never records completion, stop once and copy those details with the visible compaction stage.
