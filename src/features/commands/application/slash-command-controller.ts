...
import type {
  ...
  SlashParsedCommand,
  SlashSubmitPlan,
} from "../../../ui/composer/slash-command";
import {
  fixedWhitespaceArgs,
  noArgs,
  optionalPathArg,
  optionalRestOfLineArg,
  requiredRestOfLineArg,
} from "../../../ui/composer/slash-parsers";
import {
  hasNonWhitespaceOutside,
  removeTextRanges,
} from "../../../ui/composer/slash-state";
...
export class SlashCommandController {
  ...
  catalog(): SlashCommandSpec[] {
    return this.runtimeCommands.map((command) => ({
      ...
      argumentOptions: command.options
        ? () => command.options ?? []
        : undefined,
  ...
  async submit(plan: SlashSubmitPlan, onAccepted?: () => void): Promise<void> {
    ...
    for (const command of plan.commands) {
      const projected = this.runtimeCommands.find(
        (candidate) => candidate.name === command.spec.name,
      );
      if (!projected) {
        this.options.renderFeedback(
          plan.text.trim(),
          `/${command.spec.name} is no longer available.`,
        );
        return;
      }
      ...
      if (
        command.spec.surroundingTextPolicy === "forbid" &&
        hasNonWhitespaceOutside(plan.text, command.consumedRange)
      ) {
        this.options.renderFeedback(
          plan.text.trim(),
          `/${command.spec.name} must be used by itself. Remove the surrounding text and submit the command again.`,
        );
        return;
      }
    }
    ...
    try {
      for (const command of plan.commands)
        await command.spec.execute?.(command);
    } catch (error) {
    ...
    const remaining = removeTextRanges(
      plan.text,
      plan.commands.map((command) => command.consumedRange),
    );
    if (remaining.length > 0)
      await this.options.sendPrompt(remaining, plan.attachments);
  }

  private guidance(
    model: FrontendLocalCommandViewModel,
    command: SlashParsedCommand,
  ): string | null {
    if (!command.ok) {
      const missing = model.argument.missingLabel
        ? ` Provide ${model.argument.missingLabel}.`
        : "";
    ...
    if (
      model.validation === "vault-directory" &&
      value &&
      !this.options.isVaultDirectory(
        this.options.normalizeVaultDirectory(value),
      )
    ) {
  ...
  private async execute(
    ...
    if (model.retiredReplacement) {
      this.options.notify(
        `/${model.name} is retired in Chatobby. Use /${model.retiredReplacement} instead.`,
      );
    }
    switch (model.action) {
      ...
      case "create-session":
        return this.options.createSession(parsed);
      case "set-working-directory":
        return this.options.setWorkingDirectory(parsed);
      case "resume-session":
        return this.options.resumeSession();
      case "fork-session":
        return this.options.forkSession();
      case "clone-session":
        return this.options.cloneSession();
      case "reload":
        return this.options.reload();
      case "abort":
        return void (await this.options.abort());
      case "bash":
        return this.options.bash(parsed);
      case "set-model":
        return this.options.setModel(parsed);
      case "set-thinking":
        return this.options.setThinking(parsed);
      case "export-html":
        return this.options.exportSession("html", parsed);
      case "export-jsonl":
        return this.options.exportSession("jsonl", parsed);
      case "start-backend":
        return void (await this.options.startBackend());
      case "stop-backend":
        return void (await this.options.stopBackend());
    }
  }
}

function parserFor(
  command: FrontendLocalCommandViewModel,
): SlashCommandSpec["argParser"] {
  if (command.argument.kind === "none") return noArgs();
  if (command.argument.kind === "required-rest")
    return requiredRestOfLineArg(command.argument.missingLabel ?? "a value");
  if (command.argument.kind === "optional-path") return optionalPathArg();
  if (command.argument.kind === "fixed-whitespace")
    return fixedWhitespaceArgs(command.argument.count ?? 1);
  ...
  switch (command.argument.kind) {
    ...
    case "fixed-whitespace": {
      ...
      return Array.from(
        { length: count },
        (_, index) =>
          `<${count === 1 ? (label ?? "value") : `value-${index + 1}`}>`,
      ).join(" ");
    }
  }
}
